...
import { LineChart, Line as RechartLine, BarChart, Bar, PieChart, Pie as RechartPie, AreaChart, Area as RechartArea, XAxis, YAxis, CartesianGrid, Tooltip as ChartTooltip, Legend, ResponsiveContainer } from 'recharts';
...
// Guides for handling alerts
const alertGuides = {
  errors: [
    { id: 1, title: 'Hướng dẫn xử lý SKU quá trọng', category: 'Tồn kho', type: 'guide' },
    { id: 2, title: 'Cách khắc phục đơn hàng bị lỗi', category: 'Đơn hàng', type: 'guide' },
    { id: 3, title: 'Giảm tỷ lệ hủy đơn hiệu quả', category: 'Sản phẩm', type: 'case-study' },
    { id: 4, title: 'Tối ưu Conversion Rate', category: 'Sản phẩm', type: 'guide' },
    { id: 5, title: 'Quản lý SKU vượt phạm vi', category: 'Tồn kho', type: 'guide' }
  ],
  warnings: [
    { id: 6, title: 'Cảnh báo và xử lý SKU sắp hết hàng', category: 'Đơn hàng', type: 'guide' },
    { id: 7, title: 'Phân tích và cải thiện Traffic', category: 'Sản phẩm', type: 'case-study' },
    { id: 8, title: 'Kiểm soát chi phí tồn kho', category: 'Tồn kho', type: 'guide' }
  ]
};
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
                ...
                <Space direction="vertical" size={20} style={{ width: '100%' }}>
                  ...
                  >
                    {isReorderMode ? (
                      ...
                      >
                        <SortableContext items={tempMetricOrder}>
                          <Row gutter={[12, 12]}>
                  ...
                  <Card 
                    title={<Text strong style={{ fontSize: 16, color: '#2b2b2b' }}>Có thể bạn quan tâm</Text>}
      ...
      >
        {/* Info Message */}
        <Alert
          message="Bạn có thể tạo template ở phần Phân tích > Dashboard"
          type="info"
          showIcon
      ...
      <Drawer
        title={
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <ExclamationCircleOutlined style={{ color: '#D72C0D', fontSize: 18 }} />
            <div>
              <div style={{ fontSize: 16, fontWeight: 600 }}>Alert & Risks</div>
              <div style={{ fontSize: 12, fontWeight: 400, color: '#8c8c8c' }}>
      ...
      >
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
            <ExclamationCircleOutlined style={{ color: '#D72C0D', fontSize: 16 }} />
            <Text strong style={{ fontSize: 14, color: '#2b2b2b' }}>Lỗi</Text>
            <Tag 
              color="error" 
              style={{ 
                fontSize: 11,
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
            <WarningOutlined style={{ color: '#F49342', fontSize: 16 }} />
            <Text strong style={{ fontSize: 14, color: '#2b2b2b' }}>Cảnh báo</Text>
            <Tag 
              color="warning" 
              style={{ 
                fontSize: 11,
        ...
        <Divider orientation="left" style={{ fontSize: 13, marginTop: 24 }}>
          <Text type="secondary">Tài liệu hướng dẫn xử lý</Text>
        </Divider>
        ...
        <div>
          <List
            size="small"
            dataSource={[...alertGuides.errors, ...alertGuides.warnings]}
            renderItem={(item) => {
              const getIcon = (type) => {
                    ...
                    avatar={
                      <div style={{ 
                        ...
                        {React.cloneElement(getIcon(item.type), { 
                          style: { fontSize: 16, color: getColor(item.type) } 
                        })}
                      </div>
                    }
                    title={<Text style={{ fontSize: 13 }}>{item.title}</Text>}
